...
>
> - **已实施**：SenseVoiceSmall INT8（sherpa-onnx、本地 CPU）随发布包内置，§5.2 漏识别优化（自适应能量门限 + Silero VAD）也已实施。
> - **仍然有效的部分**：选型依据（§5.1 各方案错误率/RTF/体积对比）与漏识别根因分析——解释了为什么最终是这条链路。
> - **被谁取代**：现行代码即该方案的落地；后续实测见《实验报告-识别与翻译》（现行）。
>
