...
当前版本为 `v0.10.0`。本版本建立统一语言注册表和服务能力过滤，增加繁中、韩、法、德、西、俄界面，以及韩文、拉丁文字和西里尔文字 OCR 懒加载包；功能继续限定为 PC 桌面模式。
...
- 本地语音识别采用可选的 SenseVoiceSmall INT8，仅在一句话结束后识别最终原文；翻译路由根据所选语音档案过滤源语言，本次新增韩语选项，不把法、德、西、俄误标为本地支持。模型和运行库按需安装到软件目录，不进入主 Python 环境。
...
- 本地 OCR 提供中文、日文、英文、韩文、拉丁文字和西里尔文字六个懒加载模型包；繁中复用中文包，法/德/西语共享拉丁文字包，俄语使用西里尔文字包，各包可独立下载和删除。
...
新增语言的真实腾讯云双向翻译、OCR 模型和多 DPI 测试结果见 [多语言翻译质量测试报告.md](多语言翻译质量测试报告.md)。

## 打包

正式发布采用 one-folder，避免 one-file 向系统 `%TEMP%` 解压。构建命令见 [packaging/README.md](packaging/README.md)。

`v0.10.0` 构建输出到 `dist\VRCTranslate\`，实际大小会随依赖版本略有变化。构建脚本不会预置 OCR/语音模型、用户配置、日志或 API 密钥；启动构建后的 exe 才会在 `data` 中生成默认配置和日志。
