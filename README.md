...
当前版本为 `v0.9.3`。本版本修复单次 OCR 识别在同一画面包含多个翻译块时过早结束、只显示首个嵌字结果的问题；功能继续限定为 PC 桌面模式。
...
`v0.9.3` 构建输出到 `dist\VRCTranslate\`，实际大小会随依赖版本略有变化。构建脚本不会预置 OCR/语音模型、用户配置、日志或 API 密钥；启动构建后的 exe 才会在 `data` 中生成默认配置和日志。
