...
VRCTranslate 是面向 Windows 10/11、VRChat PC 桌面模式的文字翻译工具。自己的文字通过 OSC Chatbox 发送；其他玩家画面中的文字可以使用本地 RapidOCR，或由用户自行配置在线多模态模型直接识别翻译。译文可显示到独立浮窗、覆盖到识别区域，或同时使用两种方式。

项目不支持 PCVR，不注入或 Hook VRChat，不读取游戏进程内存，不使用数据库，不保存截图或聊天历史。

## 当前版本

当前版本为 `v0.7.0`。本版新增在线多模态 OCR、通用大模型低延迟策略，并修复 OCR 模型缺失提示、大选区文字遗漏、浮窗自动过期、长译文嵌字裁切、罗马音模式传递、错误反馈及快捷输入 Tab 焦点问题；语音识别仍处于延期状态。
...
- 支持 DeepL、Google Cloud Translation、Google 免费接口、腾讯翻译、OpenAI 兼容文本接口、OpenAI 兼容多模态接口及测试回显。
...
`v0.7.0` 构建输出到 `dist\VRCTranslate\`，实际大小会随依赖版本略有变化。构建脚本不会预置 OCR 模型、用户配置、日志或 API 密钥；启动构建后的 exe 才会在 `data` 中生成默认配置和日志。
