...
VRCTranslate 是面向 Windows 10/11 PC 桌面的 VRChat 翻译工具。自己的文字通过 OSC Chatbox 发送；其他玩家画面中的文字可以使用本地 RapidOCR 或在线多模态模型；其他玩家语音可以通过所选桌面进程的输出音频进行本地或在线识别，再交给独立文字翻译路由。

项目不支持 PCVR，不注入或 Hook VRChat，不读取游戏进程内存，不使用数据库，不保存截图或聊天历史。

## 当前版本

当前版本为 `v0.9.1`。本版本修复翻译档案默认端点显示和仅切换档案就误报未保存的问题，并为 SenseVoice 组件下载加入 Hugging Face 官方源与国内镜像的并行小样本测速、自动择优和失败换源；功能继续限定为 PC 桌面模式。
...
- 本地语音识别采用可选的 SenseVoiceSmall INT8，仅在一句话结束后识别最终原文；支持中、英、日自动检测，并兼顾韩语和粤语。模型和运行库按需安装到软件目录，不进入主 Python 环境。
...
安装后仍需新增一个“本地 · SenseVoiceSmall INT8”语音档案并验证。默认不会自动创建任何语音档案。
...
`v0.9.1` 构建输出到 `dist\VRCTranslate\`，实际大小会随依赖版本略有变化。构建脚本不会预置 OCR/语音模型、用户配置、日志或 API 密钥；启动构建后的 exe 才会在 `data` 中生成默认配置和日志。
