...
当前版本为 `v0.9.2`。本版本在主窗口、设置弹窗和全部悬浮窗中统一禁用 Tab/Shift+Tab 焦点切换，并为语音字幕浮窗增加可持久化缩放与新增字幕自动跳转到最新条目；功能继续限定为 PC 桌面模式。
...
`v0.9.2` 构建输出到 `dist\VRCTranslate\`，实际大小会随依赖版本略有变化。构建脚本不会预置 OCR/语音模型、用户配置、日志或 API 密钥；启动构建后的 exe 才会在 `data` 中生成默认配置和日志。
