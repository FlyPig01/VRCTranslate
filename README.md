...
当前版本为 `v0.11.0`。本版本依据多语言全链路基准结果完善翻译不变量保护、OCR 小字重试与最新帧调度、独立 OCR 模型包选择、本地语音语言约束和翻译方向建议；Google 免费翻译增加连续失败冷却提示。功能继续限定为 PC 桌面模式。
...
新增语言的短句人工检查见 [多语言翻译质量测试报告.md](多语言翻译质量测试报告.md)；九语言 OCR、本地 ASR、真实翻译接口和端到端性能结果见 [多语言全链路质量与性能测试报告.md](多语言全链路质量与性能测试报告.md)。设置页显示的方向建议仅来自这次本机通用基准，不会自动切换用户档案。

## 打包

正式发布采用 one-folder，避免 one-file 向系统 `%TEMP%` 解压。构建命令见 [packaging/README.md](packaging/README.md)。

`v0.11.0` 构建输出到 `dist\VRCTranslate\`，实际大小会随依赖版本略有变化。构建脚本不会预置 OCR/语音模型、用户配置、日志或 API 密钥；启动构建后的 exe 才会在 `data` 中生成默认配置和日志。
