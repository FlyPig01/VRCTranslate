...
VRCTranslate 是面向 Windows 10/11 PC 桌面的 VRChat 翻译工具。自己的文字通过 OSC Chatbox 发送；其他玩家画面中的文字可以使用本地 RapidOCR 或在线多模态模型；其他玩家语音可以通过所选桌面进程的输出音频进行在线识别和翻译。

项目不支持 PCVR，不注入或 Hook VRChat，不读取游戏进程内存，不使用数据库，不保存截图或聊天历史。

## 当前版本

当前版本为 `v0.8.0`。本版本新增 Windows PC 实时语音字幕、阿里云机器翻译 SDK、MSS 虚拟桌面框选和翻译档案弹窗，并继续限定为 PC 桌面模式。
...
- 可手动选择 VRChat、播放器、浏览器等任意桌面进程，捕获该进程及子进程的输出音频，经实时语音服务和独立文字翻译路由显示本地字幕。
- 实时语音识别只提供腾讯云实时语音识别和阿里云智能语音交互 NLS 两种专用档案；两者持续接收内存 PCM，中间结果更新当前字幕，最终原文只调用一次现有文字翻译路由。
- 语音服务 API 只做识别，不直接翻译。普通 Audio Transcriptions、DashScope Paraformer 和 `input_audio` Chat Completions 已退出实时语音功能，旧协议档案会在配置升级时删除。
- 实时语音档案必须通过显式连接验证才能启动；凭据、资源 ID、引擎或服务参数变化后自动回到“待验证”。
- 语音功能只支持 Windows PC，不采集麦克风、不调用 OSC、不保存音频、识别原文或译文；不支持时不会降级监听整个系统声音。
...
- 中文使用 PP-OCRv5 Server Rec，日文使用 PP-OCRv6 Medium Rec，英文使用 PP-OCRv5 Mobile Rec；三个模型包可独立下载和删除。
...
- OCR 翻译按识别轮次整体调度，不会因单轮文字块超过队列容量而只翻译一部分；连续模式不会无限积压旧画面。
- 自身消息、OCR 和语音使用三条独立翻译路由；语音的源语言、目标语言、文字翻译档案和术语库开关统一位于“翻译模型 → 翻译路由”。
- 语音字幕浮窗的置顶、原文显示、透明度、字号和条数位于主界面“语音”页，修改后自动保存。
- 日语罗马音提供“关闭 / 自动判断 / 强制转换”三种模式；自身消息默认自动判断，OCR 新配置默认关闭。
- 罗马音转换使用轻量 WanaKana 规则并补充标准 Hepburn 兼容处理，支持无空格长句，同时保护英文、URL、用户名和品牌名。
- 内置只读 VRChat 术语并支持便携用户术语；大模型使用结构化术语提示，其他在线服务使用本地占位符保护和失败回退。
- 支持 DeepL、Google Cloud Translation、Google 免费接口、腾讯翻译、阿里云机器翻译、OpenAI 兼容文本接口、支持图片理解的多模态接口及测试回显。
...
使用本地 OCR 前，进入“设置 → OCR → 本地 OCR 模型”，按识别语言点击“下载并安装”：
...
└── data\
    ...
    ├── models\ocr\             # 按需下载的中文/日文/英文 OCR 模型
...
`v0.8.0` 构建输出到 `dist\VRCTranslate\`，实际大小会随依赖版本略有变化。构建脚本不会预置 OCR 模型、用户配置、日志或 API 密钥；启动构建后的 exe 才会在 `data` 中生成默认配置和日志。
...
│   ├── audio/                 # 按 PID 捕获目标进程树的内存 PCM
│   ├── speech/                # 腾讯、阿里 NLS 实时协议、签名、能力校验与路由
