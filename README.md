...
当前版本为 `v0.6.0`。本版新增中英日三语术语库、日语罗马音处理和英文 OCR 模型，并完善了中日英 OCR 模型管理、翻译路由及设置页面。
...
正式模型位于 `data\models\ocr`；`data\cache\ocr-models` 只保存下载中的 `.part` 临时文件，成功、失败或取消后都会清理。只使用 OSC 自身消息翻译时无需安装 OCR 模型。
