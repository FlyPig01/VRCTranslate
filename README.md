...
当前版本为 `v0.9.0`。本版本新增可选的 SenseVoiceSmall INT8 本地语音识别、便携模型管理与完整性校验，并修复语音字幕刷新时的闪窗和高 DPI 尺寸异常；功能继续限定为 PC 桌面模式。
...
`v0.9.0` 构建输出到 `dist\VRCTranslate\`，实际大小会随依赖版本略有变化。构建脚本不会预置 OCR/语音模型、用户配置、日志或 API 密钥；启动构建后的 exe 才会在 `data` 中生成默认配置和日志。
