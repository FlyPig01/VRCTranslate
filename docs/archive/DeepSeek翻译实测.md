> ⚠️ **历史文档（2026-09，直连 API 口径）** —— 正文一个字未改，仅在此说明适用范围。
>
> - **本文件的测试口径**：脚本**直连 DeepSeek API**，**单次调用、单一目标语言**，
>   **不含** VAD 静音判定、**不含** OSC 发送、**不含**第二个目标语言；系统提示也是旧版（未声明源语言）。
> - **因此不能**与《翻译服务软件内实测对比》的端到端数字直接比较：
>   旧文中位约 900 ms = 模型单次调用；新文 1590 → 1031 ms = 用户按 Enter 到 VRChat 收到消息。
> - **仍然有效的部分**：思考模式对比（开启后输出 tokens 20~25 倍、耗时慢 2.3 倍，翻译不值得开）；质量样本。
> - **被谁取代**：《翻译服务软件内实测对比》（现行）。
>
