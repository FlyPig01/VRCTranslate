...
- 可拖动、可调整大小的轻量输入/字幕浮窗；两者默认宽度分别为 1240px 和 1400px，位置与大小会在退出后恢复；
- 可选的 Whisper Base Q5_1 本地识别模型，覆盖英语、日语、韩语，按需下载；
...
正式运行端不依赖 Python、PySide6 或 PyInstaller。OCR、屏幕捕获和多模态图片翻译不属于 V2 初始发布范围；指南只保留 QQ `Ctrl+Alt+F` 外部屏幕翻译说明。当前语音识别使用可选的 Whisper Base Q5_1 本地模型，模型未安装时不会回退到系统识别，文件按需放在 `%LOCALAPPDATA%\VRCTranslate\v2\models\speech`。音频采集端口和 Windows 麦克风/回环适配器已经分层，桌面端已经接通麦克风和系统回环；回环目前读取系统输出混音，VRChat 进程级过滤仍待后续完善。
...
手动测试前执行 `tests\Invoke-V2Validation.ps1`。该脚本会验证构建、分层测试、翻译页 UI 交互、浮窗拖动/缩放/布局恢复和六个桌面页面启动；模型下载不在构建阶段执行，首次使用本地识别时由语音页主动安装。

重新发布 `artifacts\manual-test` 后执行 `tests\Invoke-V2ReleaseSmoke.ps1`；它会检查发布资源、翻译档案对话框、两个浮窗的实际窗口和白色标题条，再把通过的包交给人工测试。

Release 手测包目录：`v2\artifacts\manual-test\`。可直接双击：

`v2\artifacts\manual-test\VrcTranslate.exe`

WinUI 3 项目需要 Windows App SDK NuGet 包。模型和 API 密钥不会写入源码。
