> ⚠️ **历史文档（V1 时期，2026-07-21，软件版本 v0.10.0）** —— 正文一个字未改，仅在此说明适用范围。
>
> - **本文件的测试对象**：V1 架构（含**屏幕 OCR**、Python 评测脚本），不是当前 V2 桌面应用。
> - **已失效的部分**：整章 **OCR**（V2 已无截屏识别，只走 OSC 文本）；Google 免费端点与 openai_compatible 档案（均已从产品删除）；
>   文中引用的 benchmarks/charts/*.svg 图表文件已随 V1 清理删除，链接不可用。
> - **仍然有效的部分**：本地 ASR 各方案的错误率 / RTF / 内存对比（中英日韩主线选 SenseVoice 的依据）；
>   以及「URL / 标识符需要独立于术语库的通用不变量保护」这一结论（现已在 V2 实现）。
> - **被谁取代**：语音侧见《SenseVoice本地替换实施方案》；翻译侧见《翻译服务软件内实测对比》
>   （两者口径完全不同，**不要跨文档比数字**）。
>
