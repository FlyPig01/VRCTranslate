...
# 逐段识别。输出段号/长度/段间缺口/识别文本，并报「不明去向样本占比」（方案 §5.3）。
if ($Vad) {
    ...
    foreach ($f in $fileList) {
        ...
        for ($i = 0; $i -lt $segments.Count; $i++) {
            ...
            "  段 {0}: 长 {1,7:N0}（{2,5:N2} s） 缺口 {3,5}  {4,7:N1} ms 检测={5,-9} {6}" -f ($i + 1), $len, ($len / 16000.0), $gap, $one.Ms, $one.Detected, $one.Text
            $prevEnd = $found + $len; $cursor = $found + 1
        }
        $unaccounted = $pcm.Length - $produced
        "  账目：喂入 {0:N0} 产出 {1:N0}  末尾缺口 {2:N0}  不明去向 {3:N0}（占喂入 {4:F2}%）" -f $pcm.Length, $produced, ($pcm.Length - $prevEnd), $unaccounted, ($unaccounted * 100.0 / $pcm.Length)
        ""
    }
